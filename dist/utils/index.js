...
export const buildSymbolName = (name, params) => {
    const { fill, wght, grad, opsz = 20 } = params;
    if (!name)
        throw new Error('Icon name empty');
    let iconParams = '';
    ...
    if (typeof grad === 'number' && grad !== 0) {
        if (grad.toString().match(regexp.grad)) {
            ...
            throw new Error(`grad value incorrect: ${grad}. Should match regexp ${regexp.grad}`);
        }
    }
    if (typeof fill === 'number' && fill !== 0) {
        if (fill === 1) {
            ...
            throw new Error(`wght value incorrect: ${fill}. Should be 1 or not presented`);
        }
    }
    if (!opsz.toString().match(regexp.opsz)) {
        throw new Error(`opsz value incorrect: ${opsz}. Should match regexp ${regexp.opsz}`);
    }
    return `${name}_${iconParams}_${opsz}px`.replace('__', '_');
};
export const downloadSymbolSVG = async (rawSymbolName, params) => {
    ...
    try {
        ...
        console.error(`Download error! Image name ${symbolNameWithParams}`, error?.toString());
        return;
    }
    ...
    fs.writeFile(`${targetDirectory}/${symbolNameWithParams}.svg`, svgData, (error) => {
        if (error) {
